...
impl PlaybackTrack<'_> {
    ...
    pub fn check_exists(&self) -> bool {
        (self.playback.factory.k4a_playback_check_track_exists)(
    ...
    pub fn is_builtin(&self) -> bool {
        (self.playback.factory.k4a_playback_track_is_builtin)(
            self.playback.handle,
            self.name.as_ptr(),
        )
    }
    ...
    pub fn get_codec_id(&self) -> Result<String, Error> {
        get_k4a_string(&|codec_id, codec_id_size| {
            (self.playback.factory.k4a_playback_track_get_codec_id)(
                self.playback.handle,
                self.name.as_ptr(),
                codec_id,
    ...
    pub fn get_codec_context(&self) -> Result<Vec<u8>, Error> {
        get_k4a_binary_data(&|codec_context, codec_context_size| {
            (self.playback.factory.k4a_playback_track_get_codec_context)(
                self.playback.handle,
                self.name.as_ptr(),
                codec_context,
