...
impl Camera<'_> {
    pub(crate) fn new<'a>(
        ...
        Error::from_k4a_result_t((device.api.k4a().k4a_device_start_cameras)(
            device.handle,
            configuration,
        ))
        .to_result(())?;
    ...
    pub fn get_capture(&self, timeout_in_ms: i32) -> Result<Capture, Error> {
        ...
        Error::from_k4a_wait_result_t((self.device.api.k4a().k4a_device_get_capture)(
            self.device.handle,
            &mut handle,
            timeout_in_ms,
        ))
...
impl Drop for Camera<'_> {
    fn drop(&mut self) {
        (self.device.api.k4a().k4a_device_stop_cameras)(self.device.handle);
    }
}
